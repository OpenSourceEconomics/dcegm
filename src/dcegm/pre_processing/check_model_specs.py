...
def extract_model_specs_info(model_specs):
    ...
    if "interest_rate" in model_specs:
        ...
        interest_rate = jnp.asarray(model_specs["interest_rate"])
        read_func_interest_rate = lambda params: interest_rate
        interest_rate_in_params = False
    else:
        read_func_interest_rate = lambda params: params["interest_rate"]
    ...
    if "income_shock_std" in model_specs:
        ...
        read_func_income_shock_std = lambda params: income_shock_std
        income_shock_std_in_params = False
    else:
        read_func_income_shock_std = lambda params: params["income_shock_std"]
        income_shock_std_in_params = True

    # income shock std processing ("income_shock_std")
    if "income_shock_mean" in model_specs:
        # Check if income_shock_std is a scalar
        income_shock_mean = jnp.asarray(model_specs["income_shock_mean"])
        read_func_income_shock_mean = lambda params: income_shock_mean
        income_shock_mean_in_params = False
    else:
        read_func_income_shock_mean = lambda params: params["income_shock_mean"]
