...
.. dropdown:: Model Configuration (model_config)
    ...
    - `income_shock_batch_size`: How many income-shock quadrature draws the backward induction interpolates at once. Left out it is `n_quad_points`, i.e. one block holding every draw, which is the single-pass solve: fastest, but it holds an array of `n_state_choices x n_continuous_combinations x n_wealth x n_quad_points` in memory. A smaller integer `k`, which has to divide `n_quad_points`, processes the draws in blocks of `k` and adds each block's weighted contribution to a running total, cutting that peak by a factor of `n_quad_points / k` at the cost of less parallelism; `k = 1` is the minimum-memory extreme. The solution does not depend on it, so use it when a solve does not fit on the device, not to change results.
