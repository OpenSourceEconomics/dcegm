...
- **Multiple continuous state variables** (`#197 <https://github.com/OpenSourceEconomics/dcegm/pull/197>`_ & `#198 <https://github.com/OpenSourceEconomics/dcegm/pull/198>`_)
  Currently, `dcegm` only supports a single deterministic continuous state variable. We plan to extend the solver to handle multiple deterministic continuous states, including support for the Druedahl–Jørgensen upper envelope algorithm.
